...
ssize_t BucketLineLen(FileBucket * bucket, uint64_t line)
{
    ...
    if (line - bucket->baseLine == bucket->lines) { // critical condtion
        ...
        printf("ERROR: bucket: %s, never to go this !!!", BucketToString(bucket));
        abort();
    }
    else {
        len = bucket->lineEndPos[line - bucket->baseLine] - bucket->lineEndPos[line - bucket->baseLine - 1];
...
int ReloadBuffer(int fd, FileBucket *bucket)
{
    ...
    if (fd < 0 || NULL == bucket) {
        printf("fd: %d < 0 or bucket: %p is NULL", fd, bucket);
        return -EINVAL;
    }
...
// Algorithm:
// sunday 算法: http://blog.163.com/yangfan876@126/blog/static/80612456201342205056344
...
int SearchKey(int fd, FileBucket *bucket, const char *key)
{
    ...
    if (NULL == bucket || NULL == key) {
        printf("bucket: %p or key: %p is NULL", bucket, key);
        return -1;
    }
...
int SearchLine(int fd, FileBucket *bucket, uint64_t line, char buff[], size_t buffLen)
{
    ...
    if (line < 1) {
        printf("request line: %llu less than 1", line);
        return -1;
    }
    ...
    if (line - bucket->baseLine <= bucket->lines) {
        write(1, BucketLineStart(bucket, line), BucketLineLen(bucket, line)); 
        ...
        printf("request line: %llu not found in FileBucket: %s, "
...
int main(int argc, char *argv[])
{
    ...
    for (i = 1; i < argc - 1; ++i) {
        if (0 == strcmp(argv[i], "-k")) {
            ...
            printf("unknow param: %s", argv[i]);
            return -1;
        }
    }

    if (NULL == file) {
        printf("need param: file with '-f'");
        printf("Usage: <-f file> "
                         "[-k key, keyword to search from file] "
                         "[-l line, line to print info in file]\n");
        return -1;
